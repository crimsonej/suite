...
function logDeletion(from, participant, suppressed) {
    console.log(`[${ts()}] ${suppressed ? chalk.gray('🚫 DEL suppressed') : chalk.red('🗑 DEL recovered')} | ${shortJid(from)} ← ${shortJid(participant)}`);
}

function logEdit(from, participant) {
    console.log(`[${ts()}] ${chalk.magenta('✏️ EDIT')} | ${shortJid(from)} ← ${shortJid(participant)}`);
}
...
module.exports = { logMessage, logCommand, logDeletion, logEdit };
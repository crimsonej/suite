...
async function _ytDownloadAndSend(sock, from, ctxMsg, mode, meta) {
    ...
    try {
        ...
        for (const playerClient of playerClients) {
            ...
            // Send initial progress message (only on first attempt)
            if (playerClient === playerClients[0]) {
                try {
        ...
        if (mode === 'audio') {
            ...
            if (stat.size <= MAX_VIDEO_BYTES_INLINE) {
                ...
                await sock.sendMessageResilient(from, {
                    ...
                    caption: `📹 ${safeTitle} (too large for inline video, sent as document)`
                }, { quoted: ctxMsg });
            }
        }
    } catch (err) {
...
async function handleMessages(sock, msg) {
    ...
    if (pickerPending) {
        ...
        if (pickMatch && isQuoted && isFromRequester) {
    ...
    // Public read-only commands (./help, ./menu, ./home, ./status) are allowed for anyone.
    ...
    if (!isFromMe && !isHomeUser && command !== 'help' && command !== 'menu' && command !== 'home' && command !== 'status') {
        return;
    }
        ...
        case 'yt': {
            try {
                ...
                if (isUrl(rest)) {
                    ...
                    const sentList = await sock.sendMessageResilient(from, {
                        text: listText
                    }, { quoted: msg });

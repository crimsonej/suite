...
async function handleMessages(sock, msg) {
    ...
    // Handled in index.js messages.upsert directly now.
    const protoType = msg.message?.protocolMessage?.type;
    if (protoType === 0 || protoType === 14 || protoType === 'REVOKE') {
        return; // Skip command processing for revoke messages
    }

    // Only process commands/features from our own messages
    ...
    switch (command) {
        ...
        case 'status': {
            ...
            const status = `🛡️ *Suites Status*\n\nSystem: Parrot OS / Linux\nStatus: Active\nUptime: ${uptimeStr}`;

            await sock.sendMessageResilient(from, { text: status });
            break;
        }

    ...
    if (proto) {
        ...
        if (isRevoke && originalMsg) {
            ...
            if (shouldTrigger) {
                ...
                logEdit(from, originalMsg.key.participant || from);
...
╚════════════════════════════╝`;
        ...
        case 'track': {
            ...
            try {
                if (global.initiatedTargets) {
                    global.initiatedTargets.add(targetJid);
                    // TTL: expire the probe so the target's real calls
                    // are never auto-rejected later

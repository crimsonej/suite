...
=======
// Register a capture waiter. Each waiter has a unique ID and timeout.
// This fixes the race condition where multiple track commands would all
// receive the same IP even if from different calls.
function captureIP(callId) {
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
...
=======
// Resolve only the most recent capture waiter with the captured IP.
// This prevents race conditions where multiple simultaneous track commands
// would all get the same IP. The most recent request gets priority.
function resolveAllCaptures(ip) {
    if (captureQueue.length === 0) return;

    // Only resolve the most recent waiter to avoid race conditions
    const waiter = captureQueue.pop();

    // Clear the timeout for this waiter
    clearTimeout(waiter.timeout);

    // Clear all other waiters (they will timeout naturally)
